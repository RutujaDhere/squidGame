...
let isDollLookingBackward = true;
...
class Doll{
    ...
    lookBackward() {
        gsap.to(this.doll.rotation,{duration: .45, y: -3.15})
        setTimeout(() => isDollLookingBackward = ture, 150)
    }
    lookforward() {
        gsap.to(this.doll.rotation,{duration: .45, y:0})
        setTimeout(() => isDollLookingBackward = false, 450)
    }
...
class Player {
    ...
    check() {
        if(!isDollLookingBackward && this.playerInfo.velocity > 0) {
